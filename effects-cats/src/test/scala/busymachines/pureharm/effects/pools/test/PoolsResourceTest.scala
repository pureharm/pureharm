...
final class PoolsResourceTest extends AnyFunSuite {
  ...
  /**
    ...
    *   the thread name
    */
  private def assertOnTP(expectContains: String)(implicit pos: Position): IO[String] =
    tn >>= compareThreadName(expectContains)

  private def compareThreadName(expected: String)(tn: String)(implicit pos: Position): IO[String] =
    p(s"should be on '$expected', and we are on: $tn") >> IO(assert(tn.contains(expected))) >> IO(tn)

  private val tn: IO[String] = IO(Thread.currentThread().getName)
