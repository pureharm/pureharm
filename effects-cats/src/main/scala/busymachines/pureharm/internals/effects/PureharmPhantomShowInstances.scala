...
object PureharmPhantomShowInstances {
  ...
  trait PhantomTypeCollectionInstances {
    ...
    implicit final def phantomTypeListShow[Tag, T](
      implicit ls: Show[List[T]],
    ): Show[List[T] @@ Tag] =
      ls.asInstanceOf[Show[List[T] @@ Tag]]

    implicit final def phantomTypeNEListShow[Tag, T](
      implicit ls: Show[NonEmptyList[T]],
    ): Show[NonEmptyList[T] @@ Tag] =
      ls.asInstanceOf[Show[NonEmptyList[T] @@ Tag]]

    implicit final def phantomTypeSetShow[Tag, T](
      implicit ls: Show[Set[T]],
    ): Show[Set[T] @@ Tag] =
      ls.asInstanceOf[Show[Set[T] @@ Tag]]

    implicit final def phantomTypeNESetShow[Tag, T](
      implicit ls: Show[NonEmptySet[T]],
    ): Show[NonEmptySet[T] @@ Tag] =
      ls.asInstanceOf[Show[NonEmptySet[T] @@ Tag]]

    implicit final def phantomTypeMapShow[Tag, K, V](
      implicit ls: Show[Map[K, V]],
    ): Show[Map[K, V] @@ Tag] =
      ls.asInstanceOf[Show[Map[K, V] @@ Tag]]

    implicit final def phantomTypeNEMapShow[Tag, K, V](
      implicit ls: Show[NonEmptyMap[K, V]],
  ...
  trait PhantomTypeScalaDurationInstances {
    ...
    implicit final def sdDurationPhantomTypeShow[Tag](
      implicit sh: Show[Duration],
    ): Show[Duration @@ Tag] =
      sh.asInstanceOf[Show[Duration @@ Tag]]

    implicit final def sdFiniteDurationPhantomTypeShow[Tag](
      implicit sh: Show[FiniteDuration],
    ): Show[FiniteDuration @@ Tag] =
      sh.asInstanceOf[Show[FiniteDuration @@ Tag]]

    implicit final def sdDeadlinePhantomTypeShow[Tag](
      implicit sh: Show[Deadline],
    ): Show[Deadline @@ Tag] =
  ...
  trait PhantomTypeJavaTimeInstances {
    ...
    implicit final def jtDurationPhantomTypeShow[Tag](
      implicit sh: Show[Duration],
    ): Show[Duration @@ Tag] =
      sh.asInstanceOf[Show[Duration @@ Tag]]

    implicit final def jtInstantPhantomTypeShow[Tag](
      implicit sh: Show[Instant],
    ): Show[Instant @@ Tag] =
      sh.asInstanceOf[Show[Instant @@ Tag]]

    implicit final def jtLocalDatePhantomTypeShow[Tag](
      implicit sh: Show[LocalDate],
    ): Show[LocalDate @@ Tag] =
      sh.asInstanceOf[Show[LocalDate @@ Tag]]

    implicit final def jtLocalDateTimePhantomTypeShow[Tag](
      implicit sh: Show[LocalDateTime],
    ): Show[LocalDateTime @@ Tag] =
      sh.asInstanceOf[Show[LocalDateTime @@ Tag]]

    implicit final def jtLocalTimePhantomTypeShow[Tag](
      implicit sh: Show[LocalTime],
    ): Show[LocalTime @@ Tag] =
      sh.asInstanceOf[Show[LocalTime @@ Tag]]

    implicit final def jtMonthDayPhantomTypeShow[Tag](
      implicit sh: Show[MonthDay],
    ): Show[MonthDay @@ Tag] =
      sh.asInstanceOf[Show[MonthDay @@ Tag]]

    implicit final def jtOffsetDateTimePhantomTypeShow[Tag](
      implicit sh: Show[OffsetDateTime],
    ): Show[OffsetDateTime @@ Tag] =
      sh.asInstanceOf[Show[OffsetDateTime @@ Tag]]

    implicit final def jtOffsetTimePhantomTypeShow[Tag](
      implicit sh: Show[Duration],
    ): Show[OffsetTime @@ Tag] =
      sh.asInstanceOf[Show[OffsetTime @@ Tag]]

    implicit final def jtPeriodPhantomTypeShow[Tag](
      implicit sh: Show[Period],
    ): Show[Period @@ Tag] =
    ...
    implicit final def jtZonedDateTimePhantomTypeShow[Tag](
      implicit sh: Show[ZonedDateTime],
    ): Show[ZonedDateTime @@ Tag] =
      sh.asInstanceOf[Show[ZonedDateTime @@ Tag]]

    implicit final def jtZoneOffsetPhantomTypeShow[Tag](
      implicit sh: Show[ZoneOffset],
    ): Show[ZoneOffset @@ Tag] =
  ...
  trait PhantomTypeTupleInstances {
    ...
    implicit final def tuple2PhantomTypeShow[T1, T2, Tag](
      implicit sh: Show[(T1, T2)],
    ): Show[(T1, T2) @@ Tag] =
      sh.asInstanceOf[Show[(T1, T2) @@ Tag]]

    implicit final def tuple3PhantomTypeShow[T1, T2, T3, Tag](
      implicit sh: Show[(T1, T2, T3)],
    ): Show[(T1, T2, T3) @@ Tag] =
      sh.asInstanceOf[Show[(T1, T2, T3) @@ Tag]]

    implicit final def tuple4PhantomTypeShow[T1, T2, T3, T4, Tag](
      implicit sh: Show[(T1, T2, T3, T4)],
